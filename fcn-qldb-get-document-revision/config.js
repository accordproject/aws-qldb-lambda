    ...
    constructor(event) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.blockAddress = event.blockAddress;
                if (!this.blockAddress) {
                    throw new Error(` Please specify blockAddress in the event message`)
                };
                if (!this.blockAddress.IonText) {
                    throw new Error(` Please specify blockAddress as ValueHolder with IonText. E.g.: {"IonText":"{strandId: \"KbW0e2QaN2uBUWzRYGeOCe\", sequenceNo: 650}`)
                };

                this.documentId = event.documentId;
                if (!this.documentId || !this.documentId.length) {
                    throw new Error(` Please specify documentId in the event message`)
                };
